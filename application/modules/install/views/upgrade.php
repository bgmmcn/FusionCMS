	...
	<body>
		...
		<div id="confirm" class="popup">
			...
			<div class="popup_links">
				...
				<button href="javascript:void(0)" id="confirm_hide" class="is-button rounded is-button-default" onClick="UI.hidePopup()">{{取消}}</button>
			...
			<div class="mx-auto w-full max-w-7xl">
					...
					<div class="relative flex h-16 w-full items-center justify-between px-4">
						<div class="flex items-center">
							...
							<div class="hidden items-center gap-2 ps-6 font-sans sm:flex" _lang_='{"step": "{{step}}"}' _step_>
								...
								<h2 class="text-muted-800 font-semibold dark:text-white" _step_name_>{{语言}}</h2>
							</div>
									...
									<div class="space-y-1">
										<li id="1" class="hover:bg-muted-100 dark:hover:bg-muted-700 flex w-full items-center gap-2 rounded-lg px-3 py-2 font-sans disabled:cursor-not-allowed disabled:opacity-70">
											<a href="#" class="router-link-active">
												<p class="text-muted-500 dark:text-muted-400 text-xs">步骤 1: </p>
												<h4 class="text-muted-800 text-xs font-medium dark:text-white">{{语言}}</h4>
											</a>
										</li>
										<li id="2" class="hover:bg-muted-100 dark:hover:bg-muted-700 flex w-full items-center gap-2 rounded-lg px-3 py-2 font-sans disabled:cursor-not-allowed disabled:opacity-70">
											<a href="#">
												<p class="text-muted-500 dark:text-muted-400 text-xs">步骤 2: </p>
												<h4 class="text-muted-800 text-xs font-medium dark:text-white">数据库迁移</h4>
											</a>
										</li>
									</div>
								</ul>
				...
				<div class="dark:bg-muted-900 flex flex-col items-center justify-between bg-white py-20">
								...
								<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md mt-10 mb-10">
									<aside class="flex items-center justify-center py-8 mx-auto w-full max-w-md">
												...
												<div class="mx-auto mt-4">
													<div class="border-muted-200 dark:border-muted-700 dark:bg-muted-800 relative w-full border bg-white transition-all duration-300 rounded-md shadow-muted-300/30 dark:shadow-muted-800/30 shadow-xl p-6 mt-4">
														<div class="mx-auto mb-4 max-w-xs text-center">
															<p class="font-heading text-xxs font-medium leading-normal text-white"> {{语言}}</p>
														</div>
										...
										<section class="box big step" style="display: none;">
											<h3 class="font-heading text-xl font-light leading-tight text-muted-800 dark:text-white">数据库迁移</h3>
											...
											<form name="migrate" onSubmit="upgrade.install(this); return false;" class="mx-auto min-w-lg max-w-lg space-y-12 py-8" data-baseURL="<?= base_url() ?>">
												<fieldset class="relative">
													<div class="mb-6"><p class="font-heading text-base font-medium leading-none text-white">CMS名称</p><p class="font-sans text-xs font-normal leading-normal text-muted-400">请选择您已安装的CMS版本进行升级</p></div>
													...
													<div class="my-6"><p class="font-heading text-base font-medium leading-none text-white">表名</p><p class="font-sans text-xs font-normal leading-normal text-muted-400">请选择表名进行迁移</p></div>
													<div class="grid grid-cols-12 gap-4">
														<div class="col-span-12">
															<div class="group/nui-input relative">
																<?php $first = true; foreach($statements as $cms => $data) { ?>
																	<select name="table" data-tip="表名" class="nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 focus:border-muted-300 focus:shadow-muted-300/50 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 dark:focus:shadow-muted-800/50 peer/input relative w-full border bg-white pe-12 ps-4 font-sans text-sm leading-5 focus:shadow-lg disabled:cursor-not-allowed disabled:opacity-75 rounded-xl" required <?= $cms ?> <?= (!$first) ? 'style="display: none;" disabled' : '' ?>>
													...
													<div class="my-6"><p class="font-heading text-base font-medium leading-none text-white">{{网站数据库}}</p><p class="font-sans text-xs font-normal leading-normal text-muted-400">{{网站数据库详情}}</p></div>
													<div class="grid grid-cols-12 gap-4">
														<div class="col-span-12">
															<div class="group/nui-input relative">
																<input type="text" name="db_hostname" data-tip="{{主机名}}" placeholder="{{主机名}}" class="nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 peer w-full border bg-white font-monospace transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-75 px-2 h-10 py-2 text-sm leading-5 pe-4 ps-9 rounded-xl" required />
														...
														<div class="col-span-12">
															<div class="group/nui-input relative">
																<input type="text" name="db_username" data-tip="{{用户名}}" placeholder="{{用户名}}" class="nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 peer w-full border bg-white font-monospace transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-75 px-2 h-10 py-2 text-sm leading-5 pe-4 ps-9 rounded-xl" required />
																<div class="h-10 w-10 text-muted-400 group-focus-within/nui-input:text-primary-500 absolute start-0 top-0 flex items-center justify-center transition-colors duration-300 peer-disabled:cursor-not-allowed peer-disabled:opacity-75">
														...
														<div class="col-span-12">
															<div class="group/nui-input relative">
																<input type="password" name="db_password" data-tip="{{密码}}" placeholder="{{密码}}" class="nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 peer w-full border bg-white font-monospace transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-75 px-2 h-10 py-2 text-sm leading-5 pe-4 ps-9 rounded-xl" required />
																<div class="h-10 w-10 text-muted-400 group-focus-within/nui-input:text-primary-500 absolute start-0 top-0 flex items-center justify-center transition-colors duration-300 peer-disabled:cursor-not-allowed peer-disabled:opacity-75">
														...
														<div class="col-span-12">
															<div class="group/nui-input relative">
																<input type="number" name="db_port" data-tip="{{端口}}" placeholder="{{端口}}: 3306" class="nui-focus border-muted-300 text-muted-600 placeholder:text-muted-300 dark:border-muted-700 dark:bg-muted-900/75 dark:text-muted-200 dark:placeholder:text-muted-500 dark:focus:border-muted-700 peer w-full border bg-white font-monospace transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-75 px-2 h-10 py-2 text-sm leading-5 pe-4 ps-9 rounded-xl" required />
																<div class="h-10 w-10 text-muted-400 group-focus-within/nui-input:text-primary-500 absolute start-0 top-0 flex items-center justify-center transition-colors duration-300 peer-disabled:cursor-not-allowed peer-disabled:opacity-75">
												...
												<div class="installer_navigation mt-6 flex items-center justify-between gap-2">
													<button type="button" class="prev is-button rounded is-button-default w-full">{{上一步}}</button>
													<button type="submit" class="is-button rounded bg-primary-500 dark:bg-primary-500 hover:enabled:bg-primary-400 dark:hover:enabled:bg-primary-400 text-white hover:enabled:shadow-lg hover:enabled:shadow-primary-500/50 dark:hover:enabled:shadow-primary-800/20 focus-visible:outline-primary-400/70 focus-within:outline-primary-400/70 focus-visible:bg-primary-500 active:enabled:bg-primary-500 dark:focus-visible:outline-primary-400 dark:focus-within:outline-primary-400 dark:focus-visible:bg-primary-500 dark:active:enabled:bg-primary-500 w-full">迁移!</button>
												</div>
					...
					<div class="text-center font-sans text-sm font-normal leading-normal text-muted-400">
						...
						<a href="https://github.com/FusionWowCMS/Modules" target="_blank">{{模块}}</a> -
						<a href="https://github.com/FusionWowCMS/Themes" target="_blank">{{主题}}</a> -
						<a href="https://github.com/FusionWowCMS/Themes/issues" target="_blank">{{支持}}</a>
						<p>
                            &copy; <?= date('Y') ?> FusionCMS. {{版权}}
						</p>
					</div>
				</div>
			</div>
