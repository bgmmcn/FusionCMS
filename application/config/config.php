...
|--------------------------------------------------------------------------
|
| URL 到你的 CodeIgniter 根目录。通常这将是你的基础 URL，
| 带有一个尾随斜杠：
|
|   http://example.com/
|
...
| 你的安装，但由于安全考虑，主机名将被设置
| 为 $_SERVER['SERVER_ADDR']，如果可用，否则为 localhost。
| 自动检测机制仅存在于开发期间的便利，
| 不得在生产环境中使用！
|
| 如果你需要允许多个域，请记住，这个文件仍然
| 是一个 PHP 脚本，你可以轻松地自己完成。
|
*/
...
|--------------------------------------------------------------------------
|
| 通常这将是你的 index.php 文件，除非你已经将其重命名为
| 其他名称。如果你使用 mod_rewrite 来删除页面，请设置此
| 变量，使其为空。
|
*/
...
|--------------------------------------------------------------------------
|
| 此选项允许你在 CodeIgniter 生成的所有 URL 后添加后缀。
| 有关更多信息，请参阅用户指南：
|
...
|--------------------------------------------------------------------------
|
| 这允许你指定在 URL 中允许使用哪些字符。
| 当有人尝试提交包含不允许字符的 URL 时，他们将
| 收到警告消息。
|
| 作为安全措施，强烈建议你将 URL 限制为
| 尽可能少的字符。默认情况下，只允许这些字符：a-z 0-9~%.:_-
|
| 留空以允许所有字符 - 但只有在你疯了的情况下。
|
| 配置的值实际上是一个正则表达式字符组
...
|
| 你可以选择启用标准查询字符串基于的 URL：
| example.com?who=me&what=something&where=here
|
| 选项是：TRUE 或 FALSE（布尔值）
|
| 其他项目允许你设置查询字符串“单词”，这些单词将
| 调用你的控制器及其函数：
...
| 你希望禁用它，请将 'allow_get_array' 设置为 FALSE。
|
...
|--------------------------------------------------------------------------
|
| 除非你想设置默认的 application/views/errors/ 目录以外的其他目录，否则请留空。
| 使用完整的服务器路径，并带有斜杠。
|
*/
...
|--------------------------------------------------------------------------
|
| 除非你想设置默认的 application/cache/ 目录以外的其他目录，否则请留空。
| 使用完整的服务器路径，并带有斜杠。
|
*/
...
|--------------------------------------------------------------------------
|
| 'cookie_prefix'   = 如果你需要避免冲突，可以设置 cookie 名称前缀
